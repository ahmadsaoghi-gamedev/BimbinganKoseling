            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-lg font-semibold">Form Tambah Bimbingan Lanjutan</h3>
                    ...
                    <!-- Form -->
                    <form action="{{ route('gurubk.bimbingan-lanjutan.store') }}" method="POST" class="space-y-6">
                        @csrf
                        ...
                        <div>
                            ...
                            <textarea name="masalah" id="masalah" rows="4" required
                                      ...
                                      class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">{{ old('masalah') }}</textarea>
                            @error('masalah')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="solusi" id="solusi" rows="4" required
                                      ...
                                      class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">{{ old('solusi') }}</textarea>
                            @error('solusi')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="tindak_lanjut" id="tindak_lanjut" rows="3" required
                                      ...
                                      class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">{{ old('tindak_lanjut') }}</textarea>
                            @error('tindak_lanjut')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
