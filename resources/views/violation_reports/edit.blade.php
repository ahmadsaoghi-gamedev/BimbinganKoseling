...
@section('title', 'Edit Rekap Pelanggaran')
            ...
            <div class="p-6 text-gray-900 dark:text-gray-100">
                ...
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
                        Edit Rekap Pelanggaran
                    </h2>
                    <a href="{{ route('violation-reports.show', $violationReport) }}" 
                       class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                        Kembali
                    </a>
                </div>

                <form action="{{ route('violation-reports.update', $violationReport) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="siswa_id" id="siswa_id" required 
                                ...
                                @foreach($siswas as $siswa)
                                    <option value="{{ $siswa->id }}" {{ $violationReport->siswa_id == $siswa->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="gurubk_id" id="gurubk_id" required 
                                ...
                                @foreach($guruBks as $guruBk)
                                    <option value="{{ $guruBk->id }}" {{ $violationReport->gurubk_id == $guruBk->id ? 'selected' : '' }}>
                                        {{ $guruBk->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="category" id="category" required 
                                ...
                                @foreach(App\Models\ViolationReport::CATEGORIES as $key => $value)
                                    <option value="{{ $key }}" {{ $violationReport->category == $key ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="date" name="violation_date" id="violation_date" 
                                   value="{{ $violationReport->violation_date ? $violationReport->violation_date->format('Y-m-d') : '' }}" required
                                   class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                        ...
                        <div>
                            ...
                            <input type="number" name="points_before" id="points_before" 
                                   value="{{ $violationReport->points_before ?? 0 }}" min="0" max="100"
                                   class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                            @error('points_before')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" name="points_after" id="points_after" 
                                   value="{{ $violationReport->points_after ?? 0 }}" min="0" max="100" required
                                   class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                            @error('points_after')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="status" id="status" required 
                                ...
                                @foreach(App\Models\ViolationReport::STATUSES as $key => $value)
                                    <option value="{{ $key }}" {{ $violationReport->status == $key ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mt-6">
                        ...
                        <textarea name="violation_description" id="violation_description" rows="4" required
                                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white"
                                  placeholder="Jelaskan detail pelanggaran yang dilakukan...">{{ $violationReport->violation_description }}</textarea>
                    ...
                    <div class="mt-6">
                        ...
                        <textarea name="sanctions" id="sanctions" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white"
                                  placeholder="Sanksi yang diberikan...">{{ $violationReport->sanctions }}</textarea>
                        @error('sanctions')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mt-6">
                        ...
                        <textarea name="prevention_actions" id="prevention_actions" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white"
                                  placeholder="Tindakan pencegahan yang akan dilakukan...">{{ $violationReport->prevention_actions }}</textarea>
                    ...
                    <div class="mt-6">
                        ...
                        <textarea name="special_notes" id="special_notes" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white"
                                  placeholder="Catatan khusus atau informasi tambahan...">{{ $violationReport->special_notes }}</textarea>
                        @error('special_notes')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="flex items-center">
                            <input type="checkbox" name="parent_notification_sent" id="parent_notification_sent" 
                                   value="1" {{ $violationReport->parent_notification_sent ? 'checked' : '' }}
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" name="summon_letter_sent" id="summon_letter_sent" 
                                   value="1" {{ $violationReport->summon_letter_sent ? 'checked' : '' }}
                                   class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
